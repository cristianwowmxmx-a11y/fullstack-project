      ...
      <div style={{
        ...
        <div onClick={() => { navigate("/"); setMenuOpen(false); }} style={{
          ...
          <div>
            ...
            <p style={{
              color: "white", fontSize: isMobile ? 13 : 15,
        ...
        {!isMobile && (
          <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
            {!isAuthenticated ? (
                ...
                {links.map(l => (
                  <button key={l.label} onClick={() => navigate(l.path)}
                    ...
                    style={{
                      ...
                      fontSize: 14, fontWeight: isActive(l.path) ? 700 : 500,
                      borderBottom: isActive(l.path) ? "2px solid #3b82f6" : "2px solid transparent",
                      paddingBottom: 2,
                    }}>
                    {l.label}
                  </button>
              ...
              <>
                <span style={{ color: "#3b82f6", fontSize: 14 }}>👤 {username}</span>
      ...
      {isMobile && menuOpen && (
        <div style={{
          ...
          {!isAuthenticated ? (
            <>
              {links.map(l => (
                <button key={l.label} onClick={() => { navigate(l.path); setMenuOpen(false); }}
                  style={{
                    background: "none", border: "none", cursor: "pointer",
                    color: isActive(l.path) ? "#3b82f6" : "#ccc",
                    padding: "12px 0", fontSize: 15, textAlign: "left",
              ...
              <button onClick={() => { navigate("/login"); setMenuOpen(false); }} style={{
                marginTop: 12, background: "#3b82f6",
            ...
            <>
              <p style={{ color: "#3b82f6", fontSize: 13, margin: "8px 0" }}>👤 {username}</p>
              <button onClick={() => { navigate("/admin"); setMenuOpen(false); }} style={{
                background: "#1e293b", border: "none", padding: 12,
                borderRadius: 8, color: "white", cursor: "pointer",
                fontWeight: 700, fontSize: 15, width: "100%", marginBottom: 8,
              }}>
                Panel
              </button>
              <button onClick={handleLogout} style={{
                background: "#ef4444", border: "none", padding: 12,
                borderRadius: 8, color: "white", cursor: "pointer",
