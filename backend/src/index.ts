...
const subirImagen = (buffer: Buffer, carpeta: string, tipo: "image" | "raw" | "auto" = "image", ext?: string): Promise<string> => {
...
app.put("/tasks/:id", auth, async (req, res) => {
  ...
  res.json(await prisma.task.update({
    where: { id: Number(req.params.id) },
    data: { ...(completed !== undefined && { completed }), ...(title !== undefined && { title }) },
  }));
});
...
app.get("/cliente-tasks", auth, async (req, res) => {
  res.json(await prisma.clienteTask.findMany({
...
app.put("/cliente-tasks/:id", auth, async (req, res) => {
  res.json(await prisma.clienteTask.update({
    where: { id: Number(req.params.id) },
...
app.post("/magazines", auth, async (req, res) => {
  ...
  res.json(await prisma.magazine.create({
    data: { title, directorId, notas: notas || null, clienteId: clienteId ? Number(clienteId) : null },
...
app.put("/magazines/:id", auth, async (req, res) => {
  ...
  res.json(await prisma.magazine.update({
    where: { id: Number(req.params.id) },
    data: { title, directorId: director.id, notas: notas !== undefined ? notas : undefined, clienteId: clienteId !== undefined ? (clienteId ? Number(clienteId) : null) : undefined },
    include: { director: true, cliente: true },
  }));
});
...
app.post("/articles", auth, async (req, res) => {
  ...
  res.json(await prisma.article.create({
    data: { title, magazineId: Number(magazineId), authors: { connect: ids.map((id: number) => ({ id })) } },
...
app.put("/articles/:id", auth, async (req, res) => {
  ...
  let person = await prisma.person.findFirst({ where: { name: authorName } });
  if (!person) person = await prisma.person.create({ data: { name: authorName } });
  res.json(await prisma.article.update({
    where: { id: Number(req.params.id) },
...
app.post("/books", auth, async (req, res) => {
  ...
  res.json(await prisma.book.create({
    data: { title, authorId: author.id, notas: notas || null, clienteId: clienteId ? Number(clienteId) : null },
...
app.put("/books/:id", auth, async (req, res) => {
  ...
  res.json(await prisma.book.update({
    where: { id: Number(req.params.id) },
    data: { title, authorId: author.id, notas: notas !== undefined ? notas : undefined, clienteId: clienteId !== undefined ? (clienteId ? Number(clienteId) : null) : undefined },
    include: { author: true, cliente: true },
  }));
});
...
app.post("/clients/form/:token/fotos", upload.fields([
...
app.put("/clients/form/:token", async (req, res) => {
  ...
  const updated = await prisma.client.update({
    ...
    data: {
      ci, nombreCompleto, direccion, fechaNacimiento, extension,
      profesion, celular, email, pideLibros, cantLibros,
      pideArticulos, cantArticulos, pideDirector, pideFundador,
      notasServicio, status: "formulario llenado",
    },
  });

  // Borrar tareas anteriores y recrear siempre
// Borrar tareas y entrega anteriores y recrear siempre
...
app.get("/entregas", auth, async (req, res) => {
  res.json(await prisma.entrega.findMany({
    orderBy: { createdAt: "desc" },
...
app.put("/entregas/:id", auth, async (req, res) => {
  ...
  res.json(await prisma.entrega.update({
    where: { id: Number(req.params.id) },
    data: { estado, fechaEntrega: estado === "entregado" ? new Date() : null },
    include: { cliente: { include: { clienteTasks: true } } },
  }));
});
